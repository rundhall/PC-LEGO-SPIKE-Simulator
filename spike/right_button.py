...
class Right_button:
	...
	def __init__(self):
		self.buttonlast = 0
		if(self.ISDEBUG):print("Right button is initialised in debug mode. Simulation Button type:",self.BUTTONTYPE ," change at spike.right_button.py ")
		self.time = time.time()
	...
	def wait_until_pressed(self):
		...
		if(self.BUTTONTYPE == "Random"):
			...
			self.time = time.time()
			while time.time() - self.time < 1000 * self.BUTTONPRESSTIME:
				if(self.ISDEBUG):print("Time remaining until simulated right button press :", str(1000 * self.BUTTONPRESSTIME - (time.time() - self.time)))
	...
	def wait_until_released(self):
		...
		if(self.BUTTONTYPE == "Random"):
			...
			self.time = time.time()
			while time.time() - self.time < 1000 * self.BUTTONPRESSTIME:
				if(self.ISDEBUG):print("Time remaining until simulated right button released :", str(1000 * self.BUTTONPRESSTIME - (time.time() - self.time)))
		elif(self.BUTTONTYPE == "Consol"):
	...
	def was_pressed(self):
		...
		if(self.BUTTONTYPE == "Random"):
			...
			if time.time() - self.time > 1000 * self.BUTTONPRESSTIME:
				if(self.ISDEBUG):print("Time is up and the simulated right button is pressed time ago:", str((time.time() - self.time)-1000 * self.BUTTONPRESSTIME))
				self.time = time.time()
				return True
			else:
				if(self.ISDEBUG):print("Simulated right button press will come in :", str(1000 * self.BUTTONPRESSTIME- (time.time() - self.time)), " sec")
				return False
